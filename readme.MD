...
`ls -al /usr/local/sbin/ && ls -al /root && head -n100 /etc/letsencrypt/live/bundle.txt -n12 /root/installssl.pl && echo "You can check these files and directory listings to ensure that Let's Encrypt is successfully installed."`
...
`sudo rm -rf /root/letsencrypt /root/installssl.pl /etc/letsencrypt/live/bundle.txt /usr/local/sbin/letencrypt* && echo "Uninstall Complete"`
